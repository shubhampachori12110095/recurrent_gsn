...
from utils.utils import cast32, logit, trunc, get_shared_weights, get_shared_bias, salt_and_pepper, \
    make_time_units_string, get_activation_function, get_cost_function, raise_to_list, closest_to_square_factors, \
    copy_params, restore_params
...
class RNN_GSN():
    ...
    def __init__(self, train_X=None, train_Y=None, valid_X=None, valid_Y=None, test_X=None, test_Y=None, args=None,
        ...
        self.layer_sizes = [self.N_input] + [
                                                self.hidden_size] * self.layers  # layer sizes, from h0 to hK (h0 is the visible layer)
        ...
        self.weights_list = [
            ...
            i in range(self.layers)]  # initialize each layer to uniform sample from sqrt(6. / (n_in + n_out))
        ...
        self.recurrent_to_gsn_weights_list = [
            ...
            layer in range(self.layers + 1) if layer % 2 != 0]
        ...
        def recurrent_step(x_t, u_tm1, add_noise):
            ...
            h_t = T.concatenate([self.hidden_activation(
                self.bias_list[i + 1] + T.dot(u_tm1, self.recurrent_to_gsn_weights_list[(i + 1) / 2])) for i in
                                 range(self.layers) if i % 2 == 0], axis=0)
        ...
        # with noise
        _, _, cost, show_cost, error = GSN.build_gsn_given_hiddens(self.Xs, h_list, self.weights_list, self.bias_list,
                                                                   True, self.noiseless_h1, self.hidden_add_noise_sigma,
                                                                   self.input_salt_and_pepper, self.input_sampling,
                                                                   self.MRG, self.visible_activation,
                                                                   self.hidden_activation, self.walkbacks,
                                                                   self.cost_function)
        # without noise for reconstruction
        x_sample_recon, _, _, recon_show_cost, _ = GSN.build_gsn_given_hiddens(self.Xs, h_list_recon, self.weights_list,
                                                                               self.bias_list, False, self.noiseless_h1,
                                                                               self.hidden_add_noise_sigma,
                                                                               self.input_salt_and_pepper,
        ...
        ################################
            ...
            if self.vis_init:
                self.bias_list[0].set_value(
                    logit(numpy.clip(0.9, 0.001, train_X[0].get_value(borrow=True).mean(axis=0))))
            ...
            while not STOP:
                ...
                log.maybeAppend(self.logger,
                                ['Train:', trunc(numpy.mean(train_costs)), trunc(numpy.mean(train_errors)), '\t'])
                ...
                if test_X is not None:
                    ...
                    log.maybeAppend(self.logger,
                                    ['Test:', trunc(numpy.mean(test_costs)), trunc(numpy.mean(test_errors)), '\t'])
            ...
            log.maybeLog(self.logger, "\n------------TOTAL RNN-GSN TRAIN TIME TOOK {0!s}---------".format(
    ...
    def sample(self, initial, n_samples=400, k=1):
        ...
        def sample_some_numbers(n_samples):
            ...
            network_state = [
                [noisy_init_vis] + [numpy.zeros((initial.shape[0], self.hidden_size), dtype='float32') for _ in
                                    self.bias_list[1:]]]
            ...
            for i in xrange(n_samples - 1):
                ...
                if i % k == 0:
                    ...
                    if i == k:
                        log.maybeLog(self.logger, "About " + make_time_units_string(
                            numpy.mean(times) * (n_samples - 1 - i)) + " remaining...")
    ...
    def plot_samples(self, epoch_number="", leading_text="", n_samples=400):
        ...
        log.maybeLog(self.logger, 'Took ' + make_time_units_string(time.time() - to_sample) + ' to sample ' + str(
            n_samples * 2) + ' numbers')
